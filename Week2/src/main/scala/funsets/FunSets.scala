...
trait FunSets extends FunSetsInterface {
   ...
   */
  def diff(s: FunSet, t: FunSet): FunSet =   x=> contains(s,x) && !contains(intersect(s,t),x)

  /**
   * Returns the subset of `s` for which `p` holds.
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = x=> contains(intersect(s,p),x)
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a < -bound) true
      else if (contains(diff(s,p),a) && contains(s,a)) false
      else iter(a-1)
    }
    iter(bound)
  }
