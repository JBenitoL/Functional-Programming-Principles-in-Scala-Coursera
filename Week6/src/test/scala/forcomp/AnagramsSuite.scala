...
class AnagramsSuite {
  ...
  @Test def `wordOccurrences: abcd (3pts)`: Unit = {
    val ca = "abcdcdcb".toLowerCase.toList.groupBy((Element: Char)=>Element).toList
    val word= "tea"
    println(ca)
    println(0%3)
    println(1%3)
    ...
    println(3%3)
    println((5%3).toInt)
    //println((ca(1)._1, ca(1)._2.length))
    println(word.toList.combinations(3).toSeq)
